...
from torchrl.data import BoundedTensorSpec, CompositeSpec, UnboundedContinuousTensorSpec, DiscreteTensorSpec, OneHotDiscreteTensorSpec, MultiOneHotDiscreteTensorSpec
...
class CarlaEnvironment(EnvBase):
    ...
    def __init__(self, client, world, town, traffic_manager, checkpoint_frequency=100, continuous_action=True, obs_size=101, device='cpu') -> None:
        ...
        # specs
        self.action_spec = MultiOneHotDiscreteTensorSpec((3, 3), dtype=torch.float) # limit the action values
...
# ----------------------------------------------------------------
    ...
    def _step(self, tensordict):
        # try:
        action = self.action_spec.to_categorical(tensordict["action"]).detach().cpu().numpy()
        self.timesteps+=1
        ...
        if action[0] == 0:
            steer = self.previous_steer + 0.1
        elif action[0] == 1:
            steer = self.previous_steer
        else:
            steer = self.previous_steer - 0.1
        if action[1] == 0:
            throttle = self.throttle - 0.1
        elif action[1] == 1:
